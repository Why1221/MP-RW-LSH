...
SRS_Cover_Tree::SRS_Cover_Tree(const char * file_path) {
  char file_path_[255];
  strcpy(file_path_, file_path);
  read_from_disk_compressed(file_path_);
}
...
void SRS_Cover_Tree::write_to_disk_compressed(const char * file_path) {
  char file_path_tmp[255];
  strcpy(file_path_tmp, file_path);
  write_to_disk_compressed(file_path_tmp);
}
