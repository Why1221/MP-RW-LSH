...
inline void writeParamFile(const char * file_path, long long n, int d, int m, int B,
                    float * proj, const char * type);  // write the para file out (as part of the index)
inline float * readParamFile(const char * index_path, long long &n, int &d, int &m, int &B,
                      char * type);  // read parameters from para file
#endif /* PARAMFILE_H_ */
