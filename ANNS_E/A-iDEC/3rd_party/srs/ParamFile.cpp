...
void writeParamFile(const char * file_path, long long n, int d, int m, int B,
                    float * proj, const char * type) {
  char file_path_tmp[255];
  strcpy(file_path_tmp, file_path);
  writeParamFile(file_path_tmp, n, d, m, B, proj, type);
}

float * readParamFile(const char * index_path, long long &n, int &d, int &m, int &B,
                      char * type) {
  char index_path_tmp[255];
  strcpy(index_path_tmp, index_path);
  return readParamFile(index_path_tmp, n, d, m, B, type);
}
