...
class SRS_Cover_Tree {
 ...
 public:
  ...
  SRS_Cover_Tree(const char * file_path);  // restore tree from disk
  ...
  void write_to_disk_compressed(const char * file_path);
  void tree_stat();  // for test only

  /// Added by Long 20190429
  size_t usedMemory() const { return sizeof(CompressedTreeNode) * (num_node - n) + sizeof(int) * n;}
};
