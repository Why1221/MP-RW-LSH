...
inline int EH3(unsigned int i0, unsigned int I1, unsigned int j)
{
  ...
  // this is not evaluated as expected, but makes no differences
  // (x & 1u) == 1u is the same as x & true
  //std::cout << (unsigned)(i0 ^ seq_xor(p_res)) <<  "\t"  << res << std::endl;
  return res;
}
...
inline unsigned int CW4B(unsigned long a, unsigned long b, unsigned long c, unsigned long d, unsigned long x, unsigned int M)
{
  unsigned int p_res = hash31(hash31(hash31(a,b,x),c,x),d,x);
