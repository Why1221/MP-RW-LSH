...
void usage() {
  ...
  printf("-c {value}     \trequired \tparameter c (# of hash "
         "bits)\n");
  ...
  printf("-d -n -ds -l -t -c -gt -qs -qn -rf -if [-k]\n");
...
indexing(const char *ds_filename, // database filename
         ...
         unsigned num_hashbits // parameter c (hash bits)
) {
  ...
  std::string perf_filename = join(
      {"falconn-indexing", "n" + std::to_string(num), "d" + std::to_string(dim),
       "l" + std::to_string(num_hashes), "c" + std::to_string(num_hashbits)},
      "-");
  perf_filename += ".txt";
  ...
  writer.writeRow(
      "s",
      "dsname,#n,#dim,#hashes,#bits,index_size(bytes),construction_time(us)");
  ...
  auto isz = getCurrentRSS();

  writer.writeRow(fmt, ds_filename, num, dim, num_hashes, num_hashbits, isz, e);
  ...
  for (unsigned i = 0; i < qn; i++) {
    ...
    for (unsigned j = 0; j < K; ++j) {
      ...
      if (!res.empty()) {
        ...
        int gdist = std::round(gt[i * r_maxk + j] * gt[i * r_maxk + j]);
        writer.writeRow(AnnResults::_DEFAULT_FMT_I_, i, j, -1, -1, gdist, -1.0f,
                        query_time);
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  unsigned num_hashes = 0, num_hashbits = 0, num_probes = 0;
  ...
  while (cnt < argc && !failed) {
    ...
    if (strcmp(para, "n") == 0) {
      ...
      if (num_hashes <= 0) {
        failed = true;
        err_msg = "num_hashes should a positive integer!";
        break;
      }
    } else if (strcmp(para, "c") == 0) {
      num_hashbits = atoi(arg);
      if (num_hashbits <= 0) {
        failed = true;
        err_msg = "c should a positive integer!";
        break;
      }
  ...
  printf("=====================================================\n");
  show_params("iiiiiiissss", "# of points", nPoints, "dimension",
              pointsDimension, "# of queries", qn, "# of hashes", num_hashes,
              "# of hash bits", num_hashbits, "# of probes", num_probes, "k", k,
  ...
  try {
    ...
    NPP_ENFORCE(num_hashes > 0 && num_probes > 0 && num_hashbits > 0 &&
                nPoints > 0 && pointsDimension > 0);
    auto table = indexing(ds, indexf, train, nPoints, pointsDimension,
                          num_hashes, num_hashbits);
