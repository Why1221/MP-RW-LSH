 ...
 public:
  ...
  void hash(const VectorType& point, std::vector<HashType>* result,
    ...
    for (int_fast32_t ii = 0; ii < l_; ++ii) {
      ...
      // Hyperplane LSH: multiply by Gaussian matrix, quantize to 1(>=0) or 0 (<0)
    }
...
//  sort the dimensions
      for (int_fast32_t ii = 0; ii < l_; ++ii) {
        HyperplaneComparator comp(hash_vector, ii * k_); // sort within the ii-th table
...
// maintain the heap structure
   ...
   private:
   // stores table, hash_mask, last_index
   // last_index is used for generating the next perturbation vector
   // if current hash_mask corresponds to (2,3), then last_index is 3,
   // the next two candidates are (2,4) and (2,3,4)
...
// The first (hash) dimension is the one closest to the hyperplane
